...
pub struct Downloader {
    ...
    forum_config_map: HashMap<String, ForumConfig>,
...
impl Downloader {
    ...
    pub async fn download_thread(&self, thread_url: &str) -> Result<()> {
        info!(
            "{}",
            self.i18n.t("thread-url", Some(&[("url", thread_url)]))
        );
        ...
        let first_page = self
            .fetch_html(client.clone(), &first_page_url, Some(mp.clone()))
            .await?;
        ...
        match forum.extract_username(&first_page) {
            ...
            None => info!(
                "{}",
                self.i18n.t("not-login", Some(&[("forum", forum.name())]))
            ),
        };
        info!(
            "{}",
        ...
        pb_spinner.set_message(self.i18n.t("initializing", None));
            ...
            .map(|pn| {
                ...
                async move {
                    let res: Result<(usize, PathBuf)> = async {
                        let doc = self
                            .fetch_html(args_clone.client.clone(), &html_url, args_clone.mp.clone())
                            .await?;
        ...
        if interval_ms > 0 {
            ...
            for (i, task) in fetch_tasks.into_iter().enumerate() {
                let current_pn = i + 2;
                pb_spinner.set_message(self.i18n.t(
                    "page-saved",
                    Some(&[
            ...
            pb_spinner.set_message(self.i18n.t("interval-ms-zero", None));
        ...
        pb_spinner.set_message(self.i18n.t("all-pages-fetched", None));
    ...
    pub async fn fetch_and_adapt_html(&self, args: &DownloadArgs) -> Result<NodeRef> {
        let document = self
            .fetch_html(args.client.clone(), &args.url_str, args.mp.clone())
            .await?;
    ...
    pub async fn fetch_html(
        ...
        let response = self.get_with_premit(client, html_url, mp).await?;
    ...
    pub async fn fetch_and_adapt_css(&self, args: &DownloadArgs) -> Result<String> {
        let response = self
            .get_with_premit(args.client.clone(), &args.url_str, args.mp.clone())
            .await?;
    ...
    async fn get_with_premit(
        ...
        let response = match mp {
            Some(ref mp) => {
                ...
                pb.set_message(self.i18n.t("downloading-asset", Some(&[("url", url)])));
