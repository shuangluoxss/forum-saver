...
use forum_saver::utils::{async_make_parent_dir, sample_config_text};
use log::{error, info};
use std::{fs, io::Write};
...
async fn main() -> anyhow::Result<()> {
    ...
    if cli.command.is_none() {
        ...
        async_make_parent_dir(&config_path).await?;
        // 检查配置文件是否存在
        ...
        if let Some(input_file) = cli.input_file {
            ...
            for url in urls {
                if let Err(e) = downloader.download_thread(&url, None).await {
                    error!(
                        "{}",
                        i18n.t(
                            "failed-download-url",
            ...
            if let Err(e) = downloader.download_thread(&thread_url, None).await {
                error!(
                    "{}",
                    i18n.t(
                        "failed-download-url",
        ...
        match command {
            forum_saver::cli::Commands::GenConfig { output_path } => {
                ...
                let sample_config_content = sample_config_text();
